...
tempdata <- climdata[climdata$Type == 'TN',]
...
dailytempdata <- gather(newtempdata, Day, Min, X1:X31)
#remove X from day name
dailytempdata$Day <- substring(dailytempdata$Day,2)
#change day name and Min to numbers
dailytempdata$Day <- as.numeric(dailytempdata$Day)
dailytempdata$Min <- as.numeric(dailytempdata$Min)
#order correctly
...
#discrete temp scale
dtd$MinDiscrete[dtd$Min < 0] <- -10
dtd$MinDiscrete[dtd$Min > 0 & dtd$Min < 10] <- 0
dtd$MinDiscrete[dtd$Min >= 10 & dtd$Min < 20] <- 10
dtd$MinDiscrete[dtd$Min >= 20 & dtd$Min < 30] <- 20
dtd$MinDiscrete[dtd$Min >= 30 & dtd$Min < 40] <- 30
dtd$MinDiscrete[dtd$Min >= 40 & dtd$Min < 50] <- 40
dtd$MinDiscrete[dtd$Min >= 50 & dtd$Min < 60] <- 50
dtd$MinDiscrete[dtd$Min >= 60 & dtd$Min < 70] <- 60
dtd$MinDiscrete[dtd$Min >= 70 & dtd$Min < 80] <- 70
...
t <- ggplot(dtd, aes(newDay, Year, fill =  as.factor(MinDiscrete))) + 
  geom_tile() + 
  #facet_grid(Year~Month) + 
  #scale_fill_discrete() +
  scale_fill_manual(values = c("pink","purple","blue4","blue1","cornflowerblue","darkseagreen","moccasin","lightsalmon","darkgoldenrod1","darkorange","white")) +
  #scale_fill_continuous(low = "linen", high = "orangered4")+
  #scale_colour_brewer(palette = "Set2") +
  labs(title = "PDX Daily Minimums 1946 - 2021",
       #subtitle = "Data up to April 2022",
       x = "Day of Year",
       y = "Year",
       fill = "Min Temp in the x's (F)") +
...
month_temp <- aggregate(Min ~ Month + Year,       # Aggregate data
                        dtd,
                        FUN = mean)
...
#discrete temp scale
month_temp$MinDiscrete[month_temp$Min < 0] <- -10
month_temp$MinDiscrete[month_temp$Min > 0 & month_temp$Min < 10] <- 0
month_temp$MinDiscrete[month_temp$Min >= 10 & month_temp$Min < 20] <- 10
month_temp$MinDiscrete[month_temp$Min >= 20 & month_temp$Min < 30] <- 20
month_temp$MinDiscrete[month_temp$Min >= 30 & month_temp$Min < 40] <- 30
month_temp$MinDiscrete[month_temp$Min >= 40 & month_temp$Min < 50] <- 40
month_temp$MinDiscrete[month_temp$Min >= 50 & month_temp$Min < 60] <- 50
month_temp$MinDiscrete[month_temp$Min >= 60 & month_temp$Min < 70] <- 60
month_temp$MinDiscrete[month_temp$Min >= 70 & month_temp$Min < 80] <- 70
month_temp$MinDiscrete[month_temp$Min >= 80 & month_temp$Min < 90] <- 80
#dtd$MinDiscrete[dtd$Min >= 90 & dtd$Min < 100] <- 90
#dtd$MinDiscrete[dtd$Min >= 100] <- 100
#dtd$MinDiscrete[dtd$Min >= 110] <- 110
...
t <- ggplot(month_temp, aes(Month, Year, fill =  as.factor(MinDiscrete))) + 
  geom_tile() + 
  #facet_grid(Year~Month) + 
  #scale_fill_discrete() +
  ...
  #scale_fill_continuous(low = "linen", high = "orangered4")+
  #scale_colour_brewer(palette = "Set2") +
  labs(title = "PDX Average Monthly Low Temperatures 1946 - 2022",
       #subtitle = "Data up to April 2022",
       x = "Day of Year",
       y = "Year",
       fill = "Average Low Temp in the x's (F)") +
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
        panel.background = element_blank(), 
        axis.line = element_blank(),
